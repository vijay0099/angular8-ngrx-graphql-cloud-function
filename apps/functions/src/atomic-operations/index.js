...
// import { firestoreInstance } from '../main';
...
//   const followerFeedRef = firestoreInstance
//     .collection(PRIV_USER_DATA)
//     .doc(followerId)
//     .collection(FEED);
//   try {
...
//     const batches = _.chunk(followedUserPosts, MAX_BATCH_SIZE).map(
//       postSnapshots => {
...
//   } catch (err) {
//     console.error(
//       'Failed updating the feed of the user ',
//       followerId,
//       'after follow user ',
//       followerId,
//       'with error',
...
//     }
//     //Generate the right amount of batches
//     const batches = _.chunk(authorFollowers, MAX_BATCH_SIZE).map(userIds => {
//       const writeBatch = firestoreInstance.batch();
//       if (isDeletion) {
...
//   }
// }
// async function updateAuthorInUserReferences(
//   userId: string,
//   newUsername: string,
//   photoUrl: string
// ) {
...
//       const postFeedsFromCurrentUser = await getPostsFromUserFeedFilterByUser(
//         userId,
//         followerId
//       );
...
//         updateReferences,
//         postFeedsFromCurrentUser.map(post =>
...
//     );
//     //Generate the right amount of batches
//     const batches = _.chunk(updateReferences, MAX_BATCH_SIZE).map(dataRefs => {
//       const writeBatch = firestoreInstance.batch();
...
//   } catch (err) {
//     console.error(
//       'There was an error trying to update the references of the user',
//       userId,
//       ' error: ',
//       err
//     );
...
//   const userPosts = await firestoreInstance
//     .collection(PRIV_USER_DATA)
//     .doc(userId)
//     .collection(AUTHOR_OF_POSTS)
//     .get();
//   return userPosts.docs.map(postSnapshot => postSnapshot.id);
// }
// async function getPostsFromUserFeedFilterByUser(
//   userId: string,
//   filterByAuthorId: string
// ): Promise<FirebaseFirestore.DocumentSnapshot[]> {
//   const feedPostsQuery = await firestoreInstance
//     .collection(PRIV_USER_DATA)
//     .doc(userId)
//     .collection(FEED)
//     .where('author.uid', '==', filterByAuthorId)
//     .get();
...
//   userId: string
// ): Promise<FirebaseFirestore.DocumentSnapshot[]> {
//   const userLikesQuery = await firestoreInstance
//     .collection(PRIV_USER_DATA)
//     .doc(userId)
//     .collection(AUTHOR_OF_LIKES)
//     .get();
//   return userLikesQuery.docs;
// }
// async function getEventsMadeByUser(
//   userId: string
// ): Promise<FirebaseFirestore.DocumentSnapshot[]> {
//   const eventsMadeByUserQuery = await firestoreInstance
...
//   userId: string
// ): Promise<FirebaseFirestore.DocumentSnapshot[]> {
//   const userCommentsQuery = await firestoreInstance
//     .collection(PRIV_USER_DATA)
//     .doc(userId)
//     .collection(AUTHOR_OF_COMMENTS)
//     .get();

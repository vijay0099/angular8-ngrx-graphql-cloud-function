...
// DATA
import { children, needs } from '../../libs/graphql-playground/data-access-data/src';
...
const renameKey = R.curry((oldKey, newKey, obj) => R.assoc(newKey, R.prop(oldKey, obj), R.dissoc(oldKey, obj)));
const findValues = (key, value) => R.when(R.has(key), x => R.pick([key], value));
...
const transformIdsToAuto = R.pipe(copyIdAsIndex, replaceIndexWithFirestoreAutoId, renameIndexToId);
// NAMING METHODS
const addAutoMethod = R.pipe(filterData, R.map(transformIdsToAuto));
const addIndexAsAutoId = R.pipe(filterData, R.map(R.pipe(copyIdAsIndex, replaceIndexWithFirestoreAutoId)));
...
const addRootCompositeMethod = R.pipe(filterData, R.map(copyIdAsIndex), setCompositeDocIdentifier(rootDocNamingDefinition));
const addNestedCompositeMethod = R.pipe(filterData, copyIdAsIndex, setCompositeDocIdentifier(nestedDocNamingDefinition));
// ROOT COLLECTIONS --------------------------------------------------------------------
...
// NESTED COLLECTIONS --------------------------------------------------------------------
const nestedIndexAuto = R.innerJoin(transformBelongsTo, R.clone(nestedCollectionDataSource), rootIndexAuto);
const nestedIndexSimple = R.innerJoin(transformBelongsTo, R.clone(nestedCollectionDataSource), rootIndexSimple);
const nestedIndexComposite = R.innerJoin(transformBelongsTo, R.clone(nestedCollectionDataSource), rootIndexComposite);

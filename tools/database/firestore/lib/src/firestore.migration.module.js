...
function filterNestedObjById(jointFieldName, index, obj) {
...
const removeKey = R.curry((keyName, data) => {
    const newData = Object.assign({}, data);
    delete newData[keyName];
    return newData;
});
const removeIndex = removeKey('index');
const setSimpleDocName = R.curry((list, obj) => R.pipe(R.pick(list), R.toPairs, R.pluck(1), R.join('_'))(obj));
const setCompositeDocName = R.curry((list, obj) => R.pipe(R.pick(list), R.toPairs, R.pluck(1), R.join('_'))(obj));
// String -> String -> {k: v}
...
const setCompositeDocIdentifier = ([p, s]) => R.map(x => R.merge(x, { index: `${R.prop(p, x)}_${R.prop(s, x)}` }));
...
const jointWith = R.curry((n, r) => R.innerJoin(transformBelongsTo, R.clone(n), r));
// NAMING METHODS
const createIndexFromFirestoreId = R.pipe(filterData, R.map(R.pipe(copyIdAsIndex, replaceIndexWithFirestoreAutoId)));
const createIndexFromId = R.pipe(filterData, R.map(copyIdAsIndex));
const createIndexFromBelongsTo = R.pipe(filterData, R.map(copyBelongsToAsIndex));
function uploadDataToFirestore({ root, nested }, db) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        const { rootCollectionDataSource, rootCollectionName, rootDocNamingMethod = 'AUTO', rootDocNamingDefinition = ['index'], rootCleaningDefinition = ['index'], hasNestedCollection = false } = root;
        const { nestedCollectionDataSource = null, nestedCollectionName = 'no_nested_collection', nestedDocNamingMethod = 'SIMPLE', nestedDocNamingDefinition = ['index'], nestedCleaningDefinition = ['index'], jointFieldName = 'index' } = nested || {};
        // OBJECTS
        ...
        // SET FLAGS
        const isNestingEnabled = !!nested && hasNestedCollection && !!nestedCollectionDataSource;
        const isRootCleaningEnabled = !!root && !!rootCleaningDefinition;
        const isNestedCleaningEnabled = !!nested &&
            hasNestedCollection &&
            !!nestedCollectionDataSource &&
            !!nestedCleaningDefinition;
        const isRootAuto = R.equals(rootDocNamingMethod, 'AUTO');
        const isRootSimple = R.equals(rootDocNamingMethod, 'SIMPLE');
        const isRootComposite = R.equals(rootDocNamingMethod, 'COMPOSITE');
        const isNestedAuto = R.and(R.equals(isNestingEnabled, true), R.equals(nestedDocNamingMethod, 'AUTO'));
        const isNestedSimple = R.and(R.equals(isNestingEnabled, true), R.equals(nestedDocNamingMethod, 'SIMPLE'));
        const isNestedComposite = R.and(R.equals(isNestingEnabled, true), R.equals(nestedDocNamingMethod, 'COMPOSITE'));
        // SET SIMPLE NAMING
        ...
        const setNestedCompositeDocName = setCompositeDocName(nestedDocNamingDefinition);
        // SET INDEX CREATION
        const createIndexCompositeId = R.pipe(filterData, R.map(copyIdAsIndex), setCompositeDocIdentifier(rootDocNamingDefinition));
        // SETTINGS ERRORS
        ...
        // LOGGING
        console.log(colors.bgGray(colors.black(`\n WRITING COLLECTION >>> "${rootCollectionName}"`)));
        ...
        Object.values(rootObj).forEach((rootNode) => tslib_1.__awaiter(this, void 0, void 0, function* () {
            let rootDocName;
            ...
            if (isNestingEnabled) {
                ...
                const filteredNestedObject = filterNestedObjById(jointFieldName, rootNode.index, nestedObj);
                ...
                filteredNestedObject.forEach((nestedNode) => tslib_1.__awaiter(this, void 0, void 0, function* () {
                    ...
                    yield nestedCollection.doc(nestedDocName).set(nestedDocument);
                }));
            }
        }));
        // SAVE TO DB
...
//# sourceMappingURL=firestore.migration.module.js.map
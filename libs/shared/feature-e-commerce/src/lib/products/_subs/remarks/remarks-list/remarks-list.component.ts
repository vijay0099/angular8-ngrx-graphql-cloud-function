...
export class RemarksListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.dataSource.entitySubject.subscribe(
      res => (this.productRemarksResult = res)
    );
  ...
  saveNewRemark() {
    ...
    this.store.dispatch(
      new ProductRemarkOnServerCreated({ productRemark: this.remarkForAdd })
    );
  ...
  editRemarkButtonOnClick(_item: ProductRemarkModel) {
    ...
    controls.editDueDate.setValue(
      this.typesUtilsService.getDateFromString(_item.dueDate)
    );
    ...
    this.store.dispatch(
      new ProductRemarkStoreUpdated({ productRemark: updateProductReamrk })
    );
    this.isSwitchedToEditMode = true;
  }
  ...
  saveUpdatedRemark(_item: ProductRemarkModel) {
    ...
    objectForUpdate.dueDate = this.typesUtilsService.getDateStringFromDate(
      _date
    );
  ...
  deleteRemarks() {
    ...
    dialogRef.afterClosed().subscribe(res => {
      ...
      this.store.dispatch(
        new ManyProductRemarksDeleted({ ids: idsForDeletion })
      );
      this.layoutUtilsService.showActionNotification(
        _deleteMessage,
        MessageType.Delete

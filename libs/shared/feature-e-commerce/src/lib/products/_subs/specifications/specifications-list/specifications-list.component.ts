...
export class SpecificationsListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.dataSource.entitySubject.subscribe(
      res => (this.productSpecificationsResult = res)
    );
  ...
  masterToggle() {
    if (this.isAllSelected()) {
      ...
      this.productSpecificationsResult.forEach(row =>
  ...
  deleteSpecs() {
    ...
    dialogRef.afterClosed().subscribe(res => {
      ...
      this.store.dispatch(
        new ManyProductSpecificationsDeleted({ ids: idsForDeletion })
      );
      this.layoutUtilsService.showActionNotification(
        _deleteMessage,
        MessageType.Delete
  ...
  addSpec() {
    ...
    dialogRef.afterClosed().subscribe(res => {
      if (res && res.isUpdated) {
        ...
        this.store.dispatch(
          new ProductSpecificationOnServerCreated({

...
export interface ProductSpecificationsState
  extends EntityState<ProductSpecificationModel> {
  productId: number;
  loading: boolean;
  totalCount: number;
...
export const adapter: EntityAdapter<ProductSpecificationModel> = createEntityAdapter<
  ProductSpecificationModel
>();
...
export function productSpecificationsReducer(
  state = initialProductSpecificationsState,
  action: ProductSpecificationActions
  ...
  switch (action.type) {
    ...
    case ProductSpecificationActionTypes.ProductSpecificationCreated:
      return adapter.addOne(action.payload.productSpecification, {
        ...state,
        lastCreatedProductSpecificationId:
          action.payload.productSpecification.id
      });
    case ProductSpecificationActionTypes.ProductSpecificationUpdated:
      return adapter.updateOne(
        action.payload.partialProductSpecification,
        state
      );
...
export const getProductRemarlState = createFeatureSelector<
  ProductSpecificationModel

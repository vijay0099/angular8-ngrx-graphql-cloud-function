...
export const adapter: EntityAdapter<ProductRemarkModel> = createEntityAdapter<
  ProductRemarkModel
>();
...
export function productRemarksReducer(
  state = initialProductRemarksState,
  action: ProductRemarkActions
): ProductRemarksState {
  switch (action.type) {
...
export const getProductRemarlState = createFeatureSelector<ProductRemarkModel>(
  'productRemarks'
);

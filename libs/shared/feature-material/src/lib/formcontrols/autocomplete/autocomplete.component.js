...
let AutocompleteComponent = class AutocompleteComponent {
    constructor(fb) {
        ...
        this.myControl33 = new FormControl();
        this.options33 = [
            new User('Mary'),
            new User('Shelley'),
            new User('Igor')
        ];
        this.myControl44 = new FormControl();
    ...
    ngOnInit() {
        ...
        this.filteredOptions33 = this.myControl33.valueChanges
            .pipe(startWith(''), map(value => typeof value === 'string' ? value : value.name), map(name => name ? this.filter33(name) : this.options33.slice()));
        this.filteredOptions44 = this.myControl44.valueChanges.pipe(startWith(''), map(val => this.filter44(val)));
        // tslint:disable-next-line:no-non-null-assertion
        this.stateGroupOptions = this.stateForm.get('stateGroup').valueChanges
            .pipe(startWith(''), map(val => this.filterGroup(val)));
    }
    filter33(name) {
...
AutocompleteComponent = tslib_1.__decorate([
    Component({
        selector: 'monorepo-autocomplete',

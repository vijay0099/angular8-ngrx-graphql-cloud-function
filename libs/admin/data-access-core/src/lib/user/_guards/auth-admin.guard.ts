...
export class AuthAdminGuard implements CanActivate {
   ...
   */
  role = '';
  constructor(private router: Router, private authService: ClientAuthService, private store: Store<AppState>) {}
    ...
    return this.getUser().pipe(
      ...
      switchMap(user => {
        ...
        if (!user) {
          this.router.navigateByUrl('/auth/login');
          return of(false);
        }else if(user.emailVerified!==true && user.providerData[0].providerId === 'Email/Password'){
          this.router.navigateByUrl('/auth/login');
          return of(false);
        }

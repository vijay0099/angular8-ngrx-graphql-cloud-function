...
export class GalleryService {
  ...
  async findGalleryById(id: string): Promise<Gallery> {
    ...
    try {
      ...
      // EXTRACT DATA
      const doc = docRef.data() as Gallery | undefined;
      ...
      return foundData as Gallery | undefined;
    } catch (error) {
      // LOGGING ERROR
  ...
  async findGalleries(): Promise<Gallery[]> {
    ...
    try {
      ...
      // RETURN TRANSFORMED DATA
      return docsData as Gallery[] | undefined;
    } catch (error) {
      // LOGGING ERROR
      console.log(colors.brightRed(error.message));
      throw new HttpException(
  ...
  // CREATE
  async createGallery(gallery: GalleryInput): Promise<Gallery> {
    try {
      const newDoc: any = {
        ...Gallery
      };
      ...
      return newDoc as Gallery | undefined;
    } catch (error) {
      throw new Error(error);
    }
  }
  
  async updateGallery(data: GalleryUpdateInput): Promise<Gallery> {
    // TODO: FINISH THIS
    try {
      // GET DATA ID
      const { id } = data;
      ...
      return updatedDoc as unknown as Gallery | undefined;
    } catch (error) {
      // LOGGING ERROR
      console.log(colors.brightRed(error.message));
  ...
  // DELETE
  async deleteGallery(id: string): Promise<Gallery> {
    // LOGGING
    console.log(colors.brightMagenta('Reading from Firestore'));
    try {
      // ERROR HANDLING
      ...
      // RETURN TRANSFORMED DATA
      return deletedDoc as Gallery | undefined;
    } catch (error) {
      throw new HttpException(
        {
          status: HttpStatus.FORBIDDEN,
